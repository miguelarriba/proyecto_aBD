...
class Rating extends EntidadBase {
  ...
  //boolean indica si al usuario le ha gustado el post
  public function isLiked($id, $mail){
    $req=$this->db()->query("SELECT * FROM likes WHERE id_post = '".$id."' AND mail = '".$mail."'");
    if($req==false)
  ...
  public function like($id, $mail){
    $query="INSERT INTO likes(id_post, mail)
            VALUES ('".$id."','".$mail."')";
  ...
  public function dislike($id, $mail){
    $req=$this->db()->query("DELETE FROM likes WHERE id_post = '".$id."' AND mail = '".$mail."'");
    if($req==false)
      throw new Exception('MySQL: Error al realizar la consulta SQL');
  }
